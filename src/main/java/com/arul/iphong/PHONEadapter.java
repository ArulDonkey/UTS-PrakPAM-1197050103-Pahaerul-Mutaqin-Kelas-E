...
public class PHONEadapter extends RecyclerView.Adapter<PHONEadapter.ListViewHolder> {
    private ArrayList<PHONE> PhoneData;

    PHONEadapter(ArrayList<PHONE> list) {
        this.PhoneData = list;
    }

    @NonNull
    @Override
    public PHONEadapter.ListViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.itemnya, parent, false);
        return new ListViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull PHONEadapter.ListViewHolder holder, int position) {
        PHONE food = PhoneData.get(position);
